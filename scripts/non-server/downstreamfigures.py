...
METRICS_DIR = os.path.join(ROOT, "output", "per_class_metrics")

MODELS = {
    "Baseline":          (f"{METRICS_DIR}/per_class_metrics_none_late.csv",        f"{PRED_DIR}/test_predictions_None_late.csv",       "Baseline"),
    "Raw (E)":           (f"{METRICS_DIR}/per_class_metrics_raw_early.csv",         f"{PRED_DIR}/test_predictions_raw_early.csv",       "Raw"),
    "Raw (L)":           (f"{METRICS_DIR}/per_class_metrics_raw_late.csv",          f"{PRED_DIR}/test_predictions_raw_late.csv",        "Raw"),
    "Wrap (E)":          (f"{METRICS_DIR}/per_class_metrics_wrap_early.csv",        f"{PRED_DIR}/test_predictions_wrap_early.csv",      "Wrap"),
    "Wrap (L)":          (f"{METRICS_DIR}/per_class_metrics_wrap_late.csv",         f"{PRED_DIR}/test_predictions_wrap_late.csv",       "Wrap"),
    "SH (E)":            (f"{METRICS_DIR}/per_class_metrics_sh_early.csv",          f"{PRED_DIR}/test_predictions_sh_early.csv",        "SH"),
    "SH (L)":            (f"{METRICS_DIR}/per_class_metrics_sh_late.csv",           f"{PRED_DIR}/test_predictions_sh_late.csv",         "SH"),
    "Hex (E)":           (f"{METRICS_DIR}/per_class_metrics_hex_early.csv",         f"{PRED_DIR}/test_predictions_hex_early.csv",       "Hex"),
    "Hex (L)":           (f"{METRICS_DIR}/per_class_metrics_hex_late.csv",          f"{PRED_DIR}/test_predictions_hex_late.csv",        "Hex"),
    "Geo_both (E)":      (f"{METRICS_DIR}/per_class_metrics_both_early.csv",        f"{PRED_DIR}/test_predictions_both_early.csv",      "Geo (both)"),
    "Geo_both (L)":      (f"{METRICS_DIR}/per_class_metrics_both_late.csv",         f"{PRED_DIR}/test_predictions_both_late.csv",       "Geo (both)"),
    "Geo_Country (E)":   (f"{METRICS_DIR}/per_class_metrics_country_early.csv",     f"{PRED_DIR}/test_predictions_country_early.csv",   "Geo (country)"),
    "Geo_Country (L)":   (f"{METRICS_DIR}/per_class_metrics_country_late.csv",      f"{PRED_DIR}/test_predictions_country_late.csv",    "Geo (country)"),
    "Geo_Continent (E)": (f"{METRICS_DIR}/per_class_metrics_continent_early.csv",   f"{PRED_DIR}/test_predictions_continent_early.csv", "Geo (continent)"),
    "Geo_Continent (L)": (f"{METRICS_DIR}/per_class_metrics_continent_late.csv",    f"{PRED_DIR}/test_predictions_continent_late.csv",  "Geo (continent)"),
}

# Best-performing geo model (used as reference throughout comparisons)
BEST_GEO = "SH (L)"
LATE_MODELS = ["Baseline", "Raw (L)", "Wrap (L)", "SH (L)", "Hex (L)", "Geo_both (L)", "Geo_Country (L)", "Geo_Continent (L)"]

# Global plot style and colour palettes
plt.rcParams.update({
...
def build_cryptic_group_sizes(species_set):
    """For each species, count how many cryptic siblings also appear in species_set."""
...
# Build summary dataframes used across multiple figures
...
def build_results_df(avail):
    ...
    for name, (m_path, _, group) in avail.items():
        ...
        sp = load_metrics(m_path)
        acc = float(raw.loc["accuracy", "f1-score"]) if "accuracy" in raw.index else sp["f1-score"].mean()
        rows.append({
            "model": name, "group": group, "accuracy": acc,
            "macro_f1": sp["f1-score"].mean(), "macro_p": sp["precision"].mean(),
            "macro_r": sp["recall"].mean(),
        })
...
def build_gains_df(avail, occ):
    ...
    df = pd.DataFrame({
        "species": common,
        "f1_base": bm.loc[common, "f1-score"].values,
        "f1_geo": gm.loc[common, "f1-score"].values,
        "f1_gain": (gm.loc[common, "f1-score"] - bm.loc[common, "f1-score"]).values,
    })
...
def main_fig1_overview(results_df, out=None):
    if out is None:
        out = os.path.join(FIG_MAIN, "fig1_model_overview.png")
    ...
    handles_sty = [
        mpatches.Patch(facecolor="#888", alpha=0.50, label="Accuracy"),
        mpatches.Patch(facecolor="#888", alpha=0.92, label="Macro-F1"),
    ]
    leg1 = ax.legend(handles=handles_col, loc="lower right", fontsize=6, framealpha=0.75,
                     title="Encoding", title_fontsize=6.5, bbox_to_anchor=(1.05, 0))
    ax.add_artist(leg1)
    ax.legend(handles=handles_sty, loc="center right", fontsize=6, framealpha=0.75,
...
def main_fig2_calibration(avail, out=None):
    if out is None:
        out = os.path.join(FIG_MAIN, "fig2_calibration.png")

    xr = np.linspace(0, 1, 400)

    model_data = {}
    for mname in LATE_MODELS:
    ...
    for ax, tag in [(ax_top, "Correct"), (ax_bot, "Incorrect")]:
        ...
        ax.text(0.97, 0.92, tag, transform=ax.transAxes, ha="right", va="top", fontsize=8,
                color="#333", bbox=dict(boxstyle="round,pad=0.25", fc="white", ec="#bbb", lw=0.5, alpha=0.92))

    ax_bot.set_xlabel("Softmax confidence", fontsize=8)
    ax_top.set_xticklabels([])
    ax_top.set_title("(A) ", fontsize=9, pad=5)
    ax_top.legend(fontsize=6.5, loc="upper left", framealpha=0.80, handlelength=1.6,
    ...
    bars = ax_gap.barh(gap_df["model"], gap_df["gap"], color=gap_df["color"].tolist(),
                       height=0.60, edgecolor="white", linewidth=0.3)

    for bar, (_, row) in zip(bars, gap_df.iterrows()):
        fw = "bold" if row["model"] == BEST_GEO else "normal"
        ax_gap.text(row["gap"] + 0.004, bar.get_y() + bar.get_height() / 2,
                    f"{row['gap']:.3f}", va="center", fontsize=7.5, fontweight=fw)
...
def main_fig3_cryptic(gains_df, out=None):
    if out is None:
        out = os.path.join(FIG_MAIN, "fig3_cryptic_analysis.png")
    ...
    ax = axes[0]
    taxon_order = (gains_df.groupby("taxon")["f1_gain"].median()
    ...
    for i, tax in enumerate(taxon_order):
        ...
        ax.plot([sub.min(), sub.max()], [i, i], color=col, lw=0.9, alpha=0.40,
    ...
    ax.set_title("(A) ", fontsize=8.5, pad=4)
    ax.set_ylim(-0.6, len(taxon_order) - 0.4)
    ax.grid(axis="x", linewidth=0.3, alpha=0.4)

    n_imp = (gains_df["f1_gain"] > 0).sum()
    pct = n_imp / len(gains_df) * 100
    ax.text(0.98, 0.02, f"{pct:.0f}% improved  (n={n_imp})", transform=ax.transAxes,
            ha="right", va="bottom", fontsize=7, color="#2ca02c",
            bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="#2ca02c", lw=0.6, alpha=0.9))
    ...
    leg_els = [
        ...
        mlines.Line2D([0], [0], marker="o", color="w", markerfacecolor="#888",
                      markeredgecolor="white", markersize=7, label="Median"),
    ]
    ...
    for idx, (_, row) in enumerate(gains_df.iterrows()):
        ...
        ax.scatter(row["f1_base"], row["f1_geo"], c=col, s=s_vals[idx], alpha=0.70,
    ...
    ax.text(0.96, 0.06, "worsened", fontsize=7, color="#d62728", transform=ax.transAxes,
            va="bottom", ha="right")
...
def main_fig4_misclassification(avail, occ, out=None):
    if out is None:
        out = os.path.join(FIG_MAIN, "fig4_misclassification.png")
    ...
    def annotate(preds):
        ...
        preds["is_cryptic_error"] = (~preds["correct"]) & preds.apply(
            lambda r: (r["True_Species"], r["Predicted_Species"]) in pairs, axis=1)
    ...
    top_pairs = (bl_errors.groupby(["True_Species", "Predicted_Species"])
                 .agg(bl_count=("True_Species", "count"))
                 .reset_index()
                 .sort_values("bl_count", ascending=False)
                 .head(10))

    fate_rows = []
    for _, row in top_pairs.iterrows():
        mask = ((bl_errors["True_Species"]    == row["True_Species"]) &
                (bl_errors["Predicted_Species"] == row["Predicted_Species"]))
        ...
        fate_rows.append({
            ...
            "sh_correct":        int(sh_outcomes["correct"].sum()),
            "sh_wrong":          int((~sh_outcomes["correct"]).sum()),
        })

    top_pairs = pd.DataFrame(fate_rows)
    top_pairs["is_cryptic"] = top_pairs.apply(
        lambda r: (r["True_Species"], r["Predicted_Species"]) in pairs, axis=1)
    ...
    for i, row in top_pairs.iterrows():
        ...
        ax.scatter(row["bl_count"], i, color=col, s=60, marker=marker, zorder=5,
                   edgecolors="#333" if row["is_cryptic"] else col, linewidths=0.8)
...
def supp_fig1_overall(results_df, out=None):
    if out is None:
        out = os.path.join(FIG_SUPP, "supp_fig1_overall.png")
    ...
    for ax, metric, title, bval in zip(
        axes,
        ["accuracy", "macro_f1"],
        ["(A)  Top-1 Accuracy", "(B)  Macro-F1"],
        [bl_acc, bl_f1],
    ):
        colors = [MODEL_COLORS.get(g, "#888") for g in df["group"]]
        ...
        for bar, val, grp in zip(bars, df[metric], df["group"]):
            ...
            ax.text(val + 0.005, bar.get_y() + bar.get_height() / 2, f"{val:.3f}",
                    va="center", fontsize=6.5, fontweight=fw)
        if bval is not None:
...
def supp_fig2_range(gains_df, out=None):
    if out is None:
        out = os.path.join(FIG_SUPP, "supp_fig2_range_breadth.png")
    if gains_df.empty:
        print(f"  [skip] {out}")
        return
    ...
    for tax, sub in gains_df.groupby("taxon"):
        ...
        ax.scatter(sub["n_conts"] + jitter, sub["f1_gain"], c=TAXON_COLORS.get(tax, "#888"),
                   s=22, alpha=0.65, linewidths=0, label=tax, zorder=3)
...
def supp_fig3_heatmap(avail, occ, out=None):
    if out is None:
        out = os.path.join(FIG_SUPP, "supp_fig3_taxon_heatmap.png")
    sp_taxon = occ.groupby("scientificName")["class"].first()
    ...
    sns.heatmap(matrix.astype(float), ax=ax, annot=True, fmt=".2f", cmap="YlGn", linewidths=0.4, linecolor="#eee",
                cbar_kws={"label": "Top-1 Accuracy", "shrink": 0.75}, annot_kws={"size": 7.5}, vmin=0.25, vmax=0.90)
    ...
    ax.set_title("Supplementary Fig. 3 — Top-1 accuracy by model × taxon", fontsize=9)
...
def supp_table_results(avail, results_df, out=None):
    if out is None:
        out = os.path.join(FIG_SUPP, "supp_table_results.tex")

    model_groups = {
        "Baseline":          ("Baseline (img only)$^\\dagger$", "—"),
        "Geo_Continent (E)": ("Geo (continent)", "Early"),
        "Geo_Continent (L)": ("Geo (continent)", "Late"),
        "Geo_Country (E)":   ("Geo (country)", "Early"),
        "Geo_Country (L)":   ("Geo (country)", "Late"),
        "Geo_both (E)":      ("Geo (both)", "Early"),
        "Geo_both (L)":      ("Geo (both)", "Late"),
        "Raw (E)":           ("Raw", "Early"),
        "Raw (L)":           ("Raw", "Late"),
        "Wrap (E)":          ("Wrap", "Early"),
        "Wrap (L)":          ("Wrap", "Late"),
    ...
    current_group = None
    for model_name, (display_name, fusion) in available_models.items():
        ...
        group_rows = results_df[results_df["group"] == display_name]
        acc_str = f"\\textbf{{{acc:.3f}}}" if acc == group_rows["accuracy"].max() else f"{acc:.3f}"
        p_str   = f"\\textbf{{{p:.3f}}}"   if p   == group_rows["macro_p"].max()   else f"{p:.3f}"
        r_str   = f"\\textbf{{{r:.3f}}}"   if r   == group_rows["macro_r"].max()   else f"{r:.3f}"
        f1_str  = f"\\textbf{{{f1:.3f}}}"  if f1  == group_rows["macro_f1"].max()  else f"{f1:.3f}"
