...
def run_experiment(encoder_type, fusion_type,
    ...
    elif encoder_type == 'geo_label' or encoder_type == "None":
...
def main():
    ...
    print(f"Classes : {len(label_enc.classes_)}")

    encodings    = resolve_encodings(args.encoding)
    fusion_types = resolve_fusion_types(args.fusion)

    unknown = [e for e in encodings if e not in (CONTINUOUS_ENCODERS | DISCRETE_ENCODERS) and e != "None"]
    if unknown:
        print(f"\nWarning: unknown encoder(s) {unknown} — skipping.")
        encodings = [e for e in encodings if e not in unknown]

    print(f"\nEncoders: {encodings}")
    print(f"Fusions : {fusion_types}")
